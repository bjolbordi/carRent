...
class DataBase
{
	...
	private function rawQuery($query)
	{
		...
		$this->debug(['query' => $query, 'timer' => $timer]);
		$this->cutStats();
		return $res;
	}
	...
	private function dbErrorLog($err)
	{
			...
			(strpos($_SERVER['HTTP_USER_AGENT'], 'YandexBot') === false))
		{
            if(!$this->conn)
                die($err);
            $this->query('INSERT INTO db_error_log SET db_error = ?s, url = ?s, http_referer = ?s, http_user_agent = ?s',
				$err, $_SERVER['REQUEST_URI'], isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '', $_SERVER['HTTP_USER_AGENT']);
		}
	...
	/**
	 * Custom method for print query stats
	 */
	private function debug($data)
	{
		if ( $this->debug ){
			print($data['query']) ;
			echo '</br> Execution Time: ';
			echo $data['timer'];
			echo '<hr>';
		}
	}
