...
/* ========================================================================== */
	...
	$(document).on('click', '.remove-image', function(e){
		e.preventDefault();
		var obj = $(this).parents('.image-div');
		...
		swal({
			...
			function(){
				$.post(url, {ajax:true, id:obj.attr('id') }, function (data) {
					var data = $.parseJSON(data);
					if(!data.Status){
						swal('', data.Message, "error", true);
					}else{
	...
	// cover image
	$(document).on('click', '.cover-image', function(e){
		e.preventDefault();
		var obj = $(this).parents('.image-div');
		var url = $(this).data('url');
		var id = obj.attr('id');
		$.post(url, {ajax:true, id:id}, function (data) {
			var data = $.parseJSON(data);
			if(data.Status){
				$('.image-div').removeClass('cover');
				$('.image-div#' + id).addClass('cover');
				var cover = $('#' + id).clone();
				if($('.image-div').length > 1){
					$('.image-div#' + id).remove();
					$('.image-div:first-child').before(cover);
				}
			}
		});
	});
